...
var manager = {
  ...
  api: function (data, onReady) {
    ...
    chrome.runtime.sendMessage({action: 'api', data: data}, onReady || manager.writeTrList);
  },
      ...
      append: (function () {
        ...
        for (var i = 0, value; value = manager.varCache.trColumnArray[i]; i++) {
          ...
          thList.push(utils.create('th', {
            ...
            title: chrome.i18n.getMessage(value.lang),
            data: {
              name: key,
              type: 'tr'
            },
            ...
            append: [
              (key === 'checkbox') ? utils.create('div', {
                ...
                text: chrome.i18n.getMessage(value.lang + '_SHORT') || chrome.i18n.getMessage(value.lang)
              }),
              resizeEl,
  ...
  getLabelOptionNode: function (item, isCustom) {
    ...
    return utils.create('option', {
      value: item,
      text: isCustom ? (item === 'SEEDING') ? chrome.i18n.getMessage('OV_FL_' + item.toUpperCase()) : chrome.i18n.getMessage('OV_CAT_' + item.toUpperCase()) : item,
  ...
  trChangeFilterByLabelBox: function () {
    ...
    chrome.storage.local.set({selectedLabel: currentLabel});
  ...
  trGetStatusInfo: function (api) {
    ...
    if (state & 32) { // paused
      if (state & 2) {
        ...
        return chrome.i18n.getMessage('OV_FL_CHECKED') + ' ' + manager.apiGetDone(api);
      } else {
        //OV_FL_PAUSED
    ...
    if (state & 1) { // started, seeding or leeching
      ...
      if (done === 1000) {
        //OV_FL_SEEDING
        status = chrome.i18n.getMessage('OV_FL_SEEDING');
      } else {
        //OV_FL_DOWNLOADING
        status = chrome.i18n.getMessage('OV_FL_DOWNLOADING');
      }
      if (!(state & 64)) {
        return "[F] " + status;
      } else {
    ...
    if (state & 16) { // error
      ...
      if (error && chrome.i18n.getMessage('lang') !== 'en' && error.substr(0, 6) === 'Error:') {
        var errMsg = chrome.i18n.getMessage('OV_FL_ERROR');
  ...
  unixTimeToTextOut: function (i) {
    ...
    var timeAgoList = chrome.i18n.getMessage('timeOutList');
      ...
      var node = utils.create('td', {
        ...
        append: utils.create('div', {
          ...
          append: [
            ...
            utils.create('a', {
              href: '#pause',
              title: chrome.i18n.getMessage('ML_PAUSE'),
              class: 'pause'
            }),
            utils.create('a', {
              href: '#stop',
  ...
  flCreateCell: {
    ...
    prio: function (columnName, api) {
      ...
      var update = function (api) {
        var priority = chrome.i18n.getMessage(priorityList[api[3]]);
        node.title = priority;
  ...
  flListShow: function (hash) {
    ...
    chrome.runtime.sendMessage({action: 'api', data: requestData}, function (data) {
      manager.writeFlList(data);
  ...
  tableResize: function (e) {
    ...
    document.body.addEventListener('mouseup', function onMouseDown(e) {
      ...
      chrome.runtime.sendMessage({
        action: 'set' + manager.capitalize(type) + 'ColumnArray',
        data: manager.varCache[type + 'ColumnArray']
      });
  ...
  updateLabesCtx: function (trigger, hash) {
    ...
    if (current_label || !hash) {
      ...
      if (items.addLabel === undefined) {
        items.addLabel = {
          name: chrome.i18n.getMessage('OV_NEW_LABEL'),
          $node: $('<li>', {'class': 'context-menu-item'}).data({
            contextMenuKey: 'addLabel',
            contextMenu: trigger.items.labels,
            contextMenuRoot: trigger
          }).append(
            $('<span>', {text: chrome.i18n.getMessage('OV_NEW_LABEL')})
          )
        };
        ...
        trigger.callbacks.addLabel = function () {
          ...
          showNotification([
            [{label: {text: chrome.i18n.getMessage('OV_NEWLABEL_CAPTION')}},
              {
                input: {
                  type: 'text', name: 'label', focus: true, on: [
                    ['keydown', function (e) {
                      if (e.keyCode === 13) {
            ...
            [
              {
                input: {
                  type: "button",
                  value: chrome.i18n.getMessage('DLG_BTN_APPLY'),
                  name: 'yesBtn',
                  on: ['click', function () {
  ...
  setSpeedDom: function (type, speed) {
    ...
    if (speedNode === undefined) {
      speedNode = manager.varCache.speedLimit[type + 'Node'] = utils.create('span', {
        'class': 'limit ' + type,
        text: value
      });
  ...
  setSpeedLimit: function (type, speed) {
    ...
    chrome.runtime.sendMessage({action: 'api', data: {action: 'setsetting', s: 'max_' + action + '_rate', v: speed}});
  ...
  setPriority: function (hash, fileIndexList, level) {
    ...
    while (fileIndexList[from] !== undefined) {
      ...
      from += step;
      chrome.runtime.sendMessage({action: 'api', data: {action: 'setprio', p: level, hash: hash, f: list}}, onReady);
    }
  },
  ...
  onLoadContextMenu: function () {
    ...
    $.contextMenu({
      ...
      items: {
        start: {
          name: chrome.i18n.getMessage('ML_START'),
          callback: function () {
        ...
        forcestart: {
          name: chrome.i18n.getMessage('ML_FORCE_START'),
          callback: function () {
        ...
        pause: {
          name: chrome.i18n.getMessage('ML_PAUSE'),
          callback: function () {
        ...
        unpause: {
          name: chrome.i18n.getMessage('resume'),
          callback: function () {
        ...
        stop: {
          name: chrome.i18n.getMessage('ML_STOP'),
          callback: function () {
        ...
        s1: '-',
        recheck: {
          name: chrome.i18n.getMessage('ML_FORCE_RECHECK'),
          callback: function () {
        ...
        remove: {
          name: chrome.i18n.getMessage('ML_REMOVE'),
          callback: function () {
            var _this = this;
            this.addClass('force');
            var list = manager.varCache.trSelectedHashList.slice(0);
            ...
            showNotification([
              [{label: {text: (!trTitle) ? chrome.i18n.getMessage('OV_CONFIRM_DELETE_MULTIPLE').replace('%d', list.length) : chrome.i18n.getMessage('OV_CONFIRM_DELETE_ONE')}},
                {span: {text: trTitle, class: 'fileName'}}],
              [
                {
                  input: {
                    type: "button", value: chrome.i18n.getMessage('DLG_BTN_YES'), on: [
                      ['click', function () {
                        this.close();
                ...
                {
                  input: {
                    type: "button", value: chrome.i18n.getMessage('DLG_BTN_NO'), focus: true, on: [
                      ['click', function () {
                        this.close();
                      }]
                    ]
        ...
        remove_with: {
          name: chrome.i18n.getMessage('ML_REMOVE_AND'),
          items: {
            remove_torrent: {
              name: chrome.i18n.getMessage('ML_DELETE_TORRENT'),
              callback: function () {
            ...
            remove_files: {
              name: chrome.i18n.getMessage('ML_DELETE_DATA'),
              callback: function () {
            ...
            remove_torrent_files: {
              name: chrome.i18n.getMessage('ML_DELETE_DATATORRENT'),
              callback: function () {
        ...
        's2': '-',
        order: {
          name: chrome.i18n.getMessage('OV_COL_ORDER'),
          items: {
            up: {
              name: chrome.i18n.getMessage('up'),
              callback: function () {
            ...
            down: {
              name: chrome.i18n.getMessage('down'),
              callback: function () {
        ...
        torrent_files: {
          name: chrome.i18n.getMessage('showFileList'),
          callback: function () {
            manager.flListShow(manager.varCache.trSelectedHashList[0]);
          }
        },
        labels: {
          name: chrome.i18n.getMessage('OV_COL_LABEL'),
          className: "labels",
          label: '',
          items: {}
        }
    ...
    $.contextMenu({
      ...
      items: {
        ...
        s1: '-',
        download: {
          name: chrome.i18n.getMessage('DLG_RSSDOWNLOADER_24'),
    ...
    $.contextMenu({
      ...
      items: function () {
        ...
        var items = {};
        items.unlimited = {
          name: chrome.i18n.getMessage('MENU_UNLIMITED'),
          speed: 0,
    ...
    $.contextMenu({
      ...
      items: function () {
        ...
        $.each(manager.varCache.trColumnList, function (key, value) {
          items[key] = {
            name: chrome.i18n.getMessage(value.lang),
  ...
  onGotFiles: function (files) {
    ...
    showNotification([
      ...
      [
        {
          input: {
            type: "button", value: chrome.i18n.getMessage('DLG_BTN_OK'), focus: true, on: [
              ['click', function () {
        ...
        {
          input: {
            type: "button", value: chrome.i18n.getMessage('DLG_BTN_CANCEL'), on: [
              ['click', function () {
                this.close();
              }]
            ]
  ...
  onLoadQuickNotification: function () {
    showNotification.selectLabelTemplate = function () {
      return [
        {label: {text: chrome.i18n.getMessage('OV_COL_LABEL')}},
        {
          select: {
            append: (function () {
              var options = [
    ...
    showNotification.selectFolderTemplate = function () {
      return [
        {label: {text: chrome.i18n.getMessage('ST_CAPT_FOLDER')}},
        {
          select: {
            append: (function () {
              var folderList = [
                $('<option>', {text: chrome.i18n.getMessage('defaultPath'), value: -1})
              ];
  ...
  onGetFreeSpace: function (freeSpace) {
    ...
    spaceItem.title = chrome.i18n.getMessage('freeSpace') + ': ' + size;
  ...
  run: function () {
    ...
    chrome.storage.local.get([
      ...
      utils.joinMessages([
        ...
        chrome.runtime.sendMessage({action: 'api', data: {action: 'getsettings'}}, function (data) {
        ...
        manager.domCache.menu.addEventListener('click', function (e) {
          ...
          if (el.classList.contains('add_magnet')) {
            ...
            showNotification([
              [
                {label: {text: chrome.i18n.getMessage('Paste_a_torrent_URL')}},
                {
                  input: {
                    type: 'text', name: 'link', focus: true, on: [
                      ['keydown', function (e) {
                        if (e.keyCode === 13) {
              ...
              [
                {
                  input: {
                    type: "button", value: chrome.i18n.getMessage('DLG_BTN_OK'), name: 'okBtn', on: [
                      ['click', function () {
                        var dataForm = this.getFormData();
                        this.close();
                        var folder = manager.varCache.folderList[dataForm.folder];
        ...
        manager.domCache.flBottom.addEventListener('click', function (e) {
          ...
          if (el.classList.contains('update')) {
            ...
            chrome.runtime.sendMessage({
              action: 'api',
              data: {action: 'getfiles', hash: manager.varCache.flListLayer.hash}
            }, function (data) {
              manager.writeFlList(data);
            });
            return;
        ...
        setTimeout(function () {
          Promise.resolve().then(() => {
            ...
            return import('../vendor/jquery.contextMenu').then(() => {
              debug && console.timeEnd('contextMenu');
              manager.onLoadContextMenu();
            });
          }).then(() => {
            debug && console.time('quickNotification');
            return import('./notifer').then((module) => {
              window.showNotification = module.default;
            ...
            if (manager.settings.showSpeedGraph) {
              ...
              return new Promise((resolve) => {
                ...
                document.body.appendChild(utils.create('script', {
                  src: require('!file-loader!../vendor/d3.min.js')
                }));
              }).then(() => {
                debug && console.timeEnd('d3js');

                return import('./graph').then((module) => {
                  window.graph = module.default;
